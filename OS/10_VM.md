...
* **Belady’s Anomaly**: For FIFO, adding more frames can cause more page faults!
    * Frames数与PF数不一定完全负相关
* 前三次必缺页
...
\>？刚换进来的只有一次reference，那不是刚换进来就出去的概率很高？是直接看大小而不是看各位bit之和，所以其实是最大的。
...
**madvise**可以通过将RO(MAP_PRIVATE)的文件映射的指针指向cow之前的内存的地址，就可以向里面写东西了